...
### ⏭️ ปุ่ม "ข้ามรอบให้อาหาร" (2026-09-18) — BE tsc + FE `vue-tsc`+`vite build` ผ่าน, ✅ สโมคเทส REST 24 ข้อผ่าน (ลบ user/ระบบทดสอบแล้ว), **ไม่ต้อง migrate**
- **ที่มา:** ผู้ใช้หาปุ่ม "ข้ามการให้อาหารวันนั้น" ไม่เจอ — ของจริงคือ **BE มี `POST /feeding-rounds/:id/skip` มาตั้งแต่ Phase 21 และ `feedingApi.skipRound()` ก็มีใน FE แล้ว แต่ไม่เคยมีปุ่มไหนเรียกเลย** (ค้นทั้ง repo แล้วไม่มี usage) → รอบนี้ต่อปุ่มให้ + เพิ่มทางข้ามล่วงหน้า
- **2 ปุ่มใน `CrabsView.vue`** (ทั้งคู่ถามยืนยันก่อน):
  - แถบรอบที่เปิดอยู่ → **"ข้ามรอบนี้"** (ข้าง "ปิดรอบเลย") = `skipRound` · ถ้าบันทึกไปบางตัวแล้ว ข้อความยืนยันเตือนว่ารอบนี้จะไม่ถูกนับในสถิติ
  - กล่องแจ้ง "ให้อาหารรอบถัดไป: …" (ยังไม่ถึงเวลา) → **"ข้ามรอบนี้"** = `skipNext` (ข้ามล่วงหน้าตั้งแต่ตอนกลางวันว่าเย็นนี้ไม่อยู่)
- **ความต่างที่ต้องสื่อให้ชัดใน UI:** *ปิดรอบเลย* = ให้อาหารแล้วแต่บันทึกไม่ครบ (นับเข้าสถิติ) · *ข้ามรอบ* = ไม่ได้ให้อาหาร → `SKIPPED`, ยกเลิก Task `FEEDING`+`SCRAP_COLLECT` ของรอบ, และ**หลุดจากหน้าวิเคราะห์อาหาร** (query ที่นั่นกรอง `status: { not: 'SKIPPED' }` อยู่แล้ว)
- **BE ใหม่ `skipUpcomingRound` + `POST /systems/:id/feeding-round/skip-next`:** จองแถวรอบของวันนั้นเป็น `SKIPPED` ทันที (ไม่สร้าง Task) แล้วเลื่อน `plan.nextDueAt` ไปรอบถัดไป → พอถึงเวลาจริง `openRound` เจอแถวเดิมจาก unique `[systemId, feedDate]` จึงไม่เปิดรอบ/ไม่สร้างงานเตือนซ้ำ (เทสข้อ 19–22 ยืนยันแล้ว)
  - ⚠️ **ห้ามเลื่อน `nextDueAt` ตอนข้ามรอบที่เปิดอยู่** (`skipRound`) — รอบที่เปิดแล้วเลื่อน `nextDueAt` ไปตั้งแต่ตอนเปิด ถ้าเลื่อนซ้ำจะ**กินวันให้อาหารหายไปอีกวัน**; `skipUpcomingRound` เลื่อนได้เพราะเป้าหมายคือ `plan.nextDueAt` เอง (= รอบที่ยังไม่เปิดเสมอ)
- **กดข้ามแล้วเปลี่ยนใจได้:** `openRoundManually` เจอรอบของวันนั้นเป็น `SKIPPED` → `reopenRound` (status กลับเป็น `OPEN` + Task ที่ `CANCELLED` กลับเป็น `PENDING`, **id รอบเดิม**) — สร้างรอบใหม่ของวันเดิมไม่ได้อยู่แล้วเพราะ unique `[systemId, feedDate]`
- **รอบที่บันทึกครบ (`COMPLETED`) ข้ามไม่ได้** (400) — ปุ่มก็ไม่โผล่อยู่แล้วเพราะแถบรอบโชว์เฉพาะ `OPEN`
- ⚠️ **WS:** อีกเครื่องได้ `feeding.progress` ที่ status ไม่ใช่ `OPEN` → `CrabsView` เรียก `loadFeeding()` ซ้ำ 1 ครั้ง เพื่อดึงวัน "รอบถัดไป" มาโชว์ (polling ผูกกับ `roundOpen` จึงหยุดไปแล้ว ไม่งั้นป้ายค้างค่าเก่าจนกว่าจะรีเฟรชหน้า)
- ⚠️ **ยังไม่ทดสอบบนเบราว์เซอร์จริง** — รอผู้ใช้เทส: กดข้ามรอบที่เปิดอยู่ (แถบหาย + ป้าย 🍤 บนกล่องหาย + งานเตือนในหน้า "งานที่ต้องทำ" กลายเป็นยกเลิก), กดข้ามล่วงหน้าแล้วดูว่าวันถัดไปที่โชว์ขยับ, กด "เริ่มบันทึกรอบนี้เลย" หลังข้ามเพื่อเรียกรอบเดิมกลับ
- ⏭️ **ยังไม่ได้ทำ (ถ้าอยากได้บอก):** กด "ข้ามงาน" ที่งาน `FEEDING` ในหน้างาน/ปฏิทิน **ยังไม่ข้ามรอบให้** (ข้ามแค่ใบงาน รอบยังเปิดค้าง) — ถ้าจะผูกต้องระวัง import วน (`task.service` ↔ `feeding.service`) และควรข้ามให้เฉพาะรอบที่ยังไม่มีใครบันทึก
...
## Log การเปลี่ยนแปลง
- **2026-09-18** — **ปุ่ม "ข้ามรอบให้อาหาร"** — BE tsc + FE build ผ่าน, ✅ สโมคเทส REST 24 ข้อผ่าน (สร้าง user/2 ระบบชั่วคราว → ลบทิ้งแล้ว), **ไม่ต้อง migrate**
  - แก้ BE: `services/feeding.service.ts` (+`markRoundSkipped`/`reopenRound`/`skipUpcomingRound`, `skipRound` บล็อกรอบ `COMPLETED`, `openRoundManually` ปลุกรอบที่ข้ามไว้), `routes/feeding.ts` (+`POST /systems/:id/feeding-round/skip-next`), `API.md`
  - แก้ FE: `views/CrabsView.vue` (ปุ่ม "ข้ามรอบนี้" 2 จุด + `skipFeedingRound`/`skipNextRound` + WS รีเฟรชแผนเมื่อรอบไม่ `OPEN`), `services/index.ts` (+`skipNext`)
  - **ที่มาของบั๊ก:** endpoint `/skip` + `feedingApi.skipRound()` มีอยู่ตั้งแต่ Phase 21 แต่ **ไม่เคยมี UI เรียก** — ผู้ใช้เลยกดข้ามไม่ได้มาตลอด
  - **gotcha ที่เจอจริง:** (1) เลื่อน `plan.nextDueAt` ได้เฉพาะตอนข้ามรอบที่ *ยังไม่เปิด* ไม่งั้นวันให้อาหารหายไปอีกวัน; (2) รอบที่มีปูตัวเดียวจะ **auto-COMPLETED ทันทีที่บันทึกตัวสุดท้าย** → ตอนเขียนเทสต้องใช้ปู 2 ตัวถึงจะเทสการข้ามรอบที่บันทึกบางส่วนได้; (3) `upsertPlan` คำนวณ `nextDueAt` เป็น "รอบถัดไปในอนาคต" เสมอ → ตั้งแผนเวลาที่ผ่านมาแล้วของวันนี้จะไม่เปิดรอบให้ (ต้องกด "เริ่มบันทึกรอบนี้เลย")
